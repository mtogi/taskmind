...
exports.sendPasswordResetEmail = exports.sendWelcomeEmail = exports.sendTaskReminder = void 0;
...
const EMAIL_TEMPLATES = {
    taskReminder: (taskTitle, dueDate, userName) => ({
        ...
        text: `Hello ${userName},\n\nThis is a reminder that your task "${taskTitle}" is due ${(0, date_fns_1.formatDistanceToNow)(dueDate, { addSuffix: true })}.\n\nTask: ${taskTitle}\nDue Date: ${dueDate.toLocaleDateString()}\n\nBest regards,\nTaskMind Team`,
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px; border: 1px solid #e2e8f0; border-top: none;">
          <p>Hello ${userName},</p>
          <p>This is a reminder that your task is due ${(0, date_fns_1.formatDistanceToNow)(dueDate, { addSuffix: true })}.</p>
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="${config_1.default.appUrl}" style="background-color: #7c3aed; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Go to TaskMind</a>
          </div>
...
const sendTaskReminder = (user, taskTitle, dueDate) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { subject, text, html } = EMAIL_TEMPLATES.taskReminder(taskTitle, dueDate, user.name);
        const info = yield transporter.sendMail({
            from: `"TaskMind" <${config_1.default.email.from}>`,
            to: user.email,
            subject,
            text,
            html,
...
// Send welcome email
const sendWelcomeEmail = (user, verificationLink) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { subject, text, html } = EMAIL_TEMPLATES.welcome(user.name, verificationLink);
        const info = yield transporter.sendMail({
            from: `"TaskMind" <${config_1.default.email.from}>`,
            to: user.email,
            subject,
...
const sendPasswordResetEmail = (user, resetToken) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const resetLink = `${config_1.default.appUrl}/reset-password?token=${resetToken}`;
        const { subject, text, html } = EMAIL_TEMPLATES.passwordReset(user.name, resetLink);
        const info = yield transporter.sendMail({
            from: `"TaskMind" <${config_1.default.email.from}>`,
            to: user.email,
            subject,
